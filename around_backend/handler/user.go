...
// this is called 安全密钥 to encrypt the token, here we use == "secret"
...
func signinHandler(w http.ResponseWriter, r *http.Request) {
	...
	var user model.User
	//deserilization: change user JSON to object
	...
	//call checkuser() with user name and passoword to check if this user has registered before
	...
	//create JWT with username info
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{
		"username": user.Username,
		"exp":      time.Now().Add(time.Hour * 24).Unix(),
	})

	//with given a signing private key and generate a whole encrypted token string
...
func signupHandler(w http.ResponseWriter, r *http.Request) {
	...
	//same as signin, change JSON to object
	decoder := json.NewDecoder(r.Body)
	var user model.User
	if err := decoder.Decode(&user); err != nil {
		http.Error(w, "Cannot decode user data from client", http.StatusBadRequest)
	...
	//if user name or password is empty or username does not belong to characer or number, return error
	...
	//if this user has already existed before
	if !success {
